...
# -------------- SHARED CONSTANTS / FUNCTIONS (from your original app) --------------

# Office (lat, lon)
office_location = (41.6586, -91.5302)
office_address_str = "322 E 2nd St, Iowa City, IA"  # text form
...
# -------------- PROPERTY INFORMATION MAP (Leaflet) --------------

def render_property_map(property_df: pd.DataFrame):
    """Leaflet property information map using Information.csv-style columns."""
...
# -------------- TAB 1: YOUR ORIGINAL ROUTING APP (wrapped, behavior unchanged) --------------
...
# -------------- TAB 2: PROPERTY INFO MAP --------------

with property_tab:
    st.title("🏠 Property Information Map")
    st.markdown("This map uses the `Information.csv` file in the same folder as this app.")

    try:
        property_df = pd.read_csv("Information.csv")
        st.dataframe(property_df.head())
        render_property_map(property_df)
    except FileNotFoundError:
        st.error("`Information.csv` not found. Place it next to this script (app.py) in your repo/server.")